...
class Transformer(nn.Module):
    def __init__(self, config, dim):
        ...
        #transformer
        seqTransEncoderLayer = nn.TransformerEncoderLayer(d_model=144, nhead=8)
        self.seqTransEncoder = nn.TransformerEncoder(seqTransEncoderLayer,
                                                     num_layers=4)

        #skeleton_emb
        self.skelEmbedding = nn.Linear(64, 144)

        self.emb = nn.Embedding(num_embeddings=6, embedding_dim=6)
        self.mlp = MLP_label(config)
    ...
    def forward(self, x, y):
        ...
        # x_seq = torch.cat((y, x), axis=0)
        x_seq = self.seqTransEncoder(x)
        x_seq = x_seq[0].unsqueeze(-1)
...
class JointGen(nn.Module):
    def __init__(self, config):
        ...
        self.content_embedding = nn.Embedding(config.emb_num, config.emb_dim)
        self.transformer = Transformer(config, "3d")
...
class PatchDis(nn.Module):
    def __init__(self, config):
        ...
        # seqTransEncoderLayer = nn.TransformerEncoderLayer(d_model=64, nhead=8)
        # self.seqTransEncoder = nn.TransformerEncoder(seqTransEncoderLayer,
        #                                              num_layers=4)
    ...
    def forward(self, x, y):
        ...
        #添加transformer层
        # x = self.seqTransEncoder(x.permute(2,0,1))
        # x = x.permute(1,2,0)

        feat = self.cnn_f(x)
        out = self.cnn_c(feat)
        index = torch.LongTensor(range(out.size(0))).to(self.device)
        # print(out.shape, y[0], index[0], out.size(0))
        out = out[index, y, :]
        return out, feat
